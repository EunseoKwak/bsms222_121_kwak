...
각 stage에 해당하는 사람의 KCN genes, SCN genes, CACN genes에 대한 expression level median을 point로 나타냈다. jitter를 이용해 점이 최대한 겹치지 않게 했다.다음으로 각 stage에 해당하는 사람의 median 평균을 선으로 이어 나타냈다. KCN genes에 대한 선 그래프를 빨간색, SCN genes에 대한 선 그래프를 파란색, CACN genes에 대한 그래프를 선 초록색으로 나타냈다. 또한 3종류의 그래프 모두에 대해 선 그래프가 나타내는 것이 Median Average per stage임을 geom_text를 이용해 나타냈다. 이후 ggtitle, labs를 이용해 title을 달고 theme을 이용해 그래프를 더 보기 좋게 했다.
...
KCN genes의 경우, stage 2A에서 stage 7까지 꾸준히 expression level이 증가함을 확인할 수 있다. Stage 8, 즉 Early childhood에 expression level이 감소했다. 이후 stage 9, 10에서 다시 증가하다가 stage 11, Adulthood에 감소했다. Stage 2A에 비해 11일 때의 expression level이 더 크다. 각 stage마다 찍힌 점들을 통해 같은 stage일 때 사람들의 gene expression level median 값이 비슷한지 차이가 있는지 확인할 수 있다. Stage 2B의 경우 점들이 몰려있는 데에 비해 Stage 7의 경우 점들이 흩어진 정도가 비교적 크다.

SCN genes의 경우, stage 2A에서 stage 7까지 꾸준히 expression level이 증가한다. Stage 8, 즉 Early Childhood에 expression level이 감소했으며 이후 stage 9, 10, 11에서 다시 증가한다. Stage 2A에 비해 11일 때의 expression level이 더 크다. KCN genes의 경우와 마찬가지로 각 stage마다 찍힌 점들을 통해 median 분포를 볼 수 있다. Stage 11에서 양 극단의 점들이 가장 멀리 떨어져있고 2B, 3A에서 점들이 비교적 몰려 있는 것을 확인할 수 있다.

KCN genes와 SCN genes의 경우 Expression rate가 developmental stage가 증가할수록 대체로 증가하는 경향성을 띤다. 반면CACN의 경우 developmental stage가 높아질수록 Expression rate가 대체로 감소한다. 2A에서 2B로 넘어갈 때 줄어들고, 3B까지 증가하며, 5까지 감소하다가 6에서 다시 증가, 8까지 감소, 9부터 11까지 증가한다. Stage 2A에 비해 11일 때의 expression level이 더 작다. 마찬가지로 점들의 분포를 통해    


추가로 각 단계에서 3 종류 유전자의 expression level 경향성을 비교해보았다.
차이를 조금 더 잘 비교할 수 있도록 y axis에 scale_y_log10을 적용했고, 위와 같은 방식으로 geom)jitter, stat_summary를 이용해 median과 median들의 평균을 나타냈다. 이후 theme_light, ggtitle, labs, themes 함수를 이용해 그래프의 aesthetics를 다뤘다.
...
점들과 선을 모두 고려해서 봤을 때, Stage 2A부터 5까지는 CACN gene들의 expression level이 가장 높음을 확인할 수 있다. Stage 3B까지 대체로 그 다음으로 KCN gene들의 expression level이 가장 높았다. Stage 9부터 11번까지는 SCN gene들의 expression level이 대체로 가장 높음을 확인할 수 있다. Stage 8에서 CACN, KCN, SCN의 expression level이 모두 감소하는 경향성을 보임을 확인할 수 있다. 